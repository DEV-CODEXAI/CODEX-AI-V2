...
module.exports = {
  ...
  category: 'utils',
  desc: 'Capture a tablet-view screenshot of a website',
  execute: async (sock, m, { args, reply }) => {
    try {
      ...
      // 3. FETCH IMAGE
      const apiUrl = `https://api-rebix.zone.id/api/ssweb?url=${encodeURIComponent(targetUrl)}&device=tablet`;
      const response = await axios.get(apiUrl, { responseType: 'arraybuffer' });
      const buffer = Buffer.from(response.data, 'utf-8');

      // 4. CLEAN BOLD CAPTION ONLY
